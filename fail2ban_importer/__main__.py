...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Make no changes, just test download and parse",
)
...
def cli(
    ...
    config = ConfigFile.model_validate(load_config(config_file=config_file))
    ...
    if not oneshot:
        ...
        schedule.every(config.schedule_mins).minutes.do(
            download_and_ban,
            download_module,
            config,
        )
