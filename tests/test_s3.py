...
def test_http_importer(tmp_path: Path) -> None:
    ...
    # write a basic config file to disk so we can use it
    config_file = {"source": "s3://fail2ban-importer-test/example.json"}
    ...
    if os.getenv("AWS_PROFILE") or (
        ...
        print("Need AWS credentials to run this test")
        pytest.skip()
